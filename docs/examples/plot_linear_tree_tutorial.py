"""
Accelerated Training for Multi-Label Classification
===================================================

Multi-label classification is an important task in machine learning that involves predicting multiple labels for a given input. 
While linear models are commonly used for this task, training on large-scale datasets with many labels can be time-consuming. 
...
To demonstrate the effectiveness of our approach, we will use the EUR-Lex dataset with 3,956 labels, 
which can be downloaded from `LIBSVM Data Sets <https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/multilabel.html>`_. 
We will provide code that is similar to `our quickstart of linear model <plot_linear_quickstart.html>`_, but with the added benefits of faster training time.
"""
...
######################################################################
# On our machine with an AMD-7950X CPU, we observed that the `train_1vsrest` function required `578.30` seconds to complete the training on the EUR-Lex dataset, 
...
#   who introduced some label approximation techniques to improve the training speed of their model, known as Bonsai. 
#   While the details of these techniques are beyond the scope of this tutorial, 
