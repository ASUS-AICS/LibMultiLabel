...
def train_1vsrest(y: sparse.csr_matrix, x: sparse.csr_matrix, options: str):
    ...
    for i in tqdm(range(num_class)):
        ...
        weights[:, i] = do_train(2*yi - 1, x, options)['weights'].reshape(-1)
...
def scutfbr(y: np.ndarray,
    ...
    for fold in range(nr_fold):
        ...
        w = do_train(y[train_idx], x[train_idx], options)['weights']
        wTx = (x[val_idx] * w).A1
        scut_b = 0.
    ...
    weights = do_train(y, x, options)['weights']
...
def cost_sensitive_one_label(y: np.ndarray,
    ...
    weights = do_train(y, x, final_options)['weights']
    return weights
...
def train_cost_sensitive_micro(y: sparse.csr_matrix, x: sparse.csr_matrix, options: str):
    ...
    for i in range(num_class):
        yi = y[:, i].toarray().reshape(-1)
        w = do_train(2*yi - 1, x, final_options)['weights']
        weights[:, i] = w.ravel()
