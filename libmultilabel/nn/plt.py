...
class PLTTrainer:
    ...
    def reformat_text(self, dataset):
        ...
        encoded_text = list(
            map(
                lambda text: torch.tensor([self.word_dict[word] for word in text], dtype=torch.long)
                if text
                else torch.tensor([self.word_dict[UNK]], dtype=torch.long),
        ...
        encoded_text[0] = torch.cat(
            (
                encoded_text[0],
                torch.tensor(0, dtype=torch.long).repeat(self.max_seq_length - encoded_text[0].shape[0]),
            )
        )
