...
def linear_test(config, model, datasets):
    ...
    assert not (config.save_positive_predictions and config.save_k_predictions > 0), """
        ...
        Save all labels with decision value larger than 0 by using save_all and save_k_predictions=0."""
    ...
    for i in tqdm(range(ceil(num_instance / config.eval_batch_size))):
        ...
        if k > 0:
            res = linear.get_labels(preds, config.save_positive_predictions, config.save_k_predictions)
            idx[slice] = res[0]
            scores[slice] = res[1]
