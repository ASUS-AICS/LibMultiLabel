...
def get_optimal_epochs(log_dir, val_metric, mode):
    """Get the number of epochs at the point of optimal validation performance.

    Args:
        log_dir (str): The directory of the trial of the experiment.
...
def prepare_retrain_config(best_config, best_log_dir, no_retrain_optimal):
    ...
    Args:
        ...
        no_retrain_optimal (bool): Do not retrain with the optimal number of epochs.
    """
    if no_retrain_optimal:
        best_config.merge_train_val = False
    else:
        best_config.merge_train_val = True
        # retraining with the optimal number of epochs
        best_config.epochs = get_optimal_epochs(best_log_dir,
                                                best_config.val_metric,
                                                best_config.mode)
...
def retrain_best_model(exp_name, best_config, best_log_dir, no_retrain_optimal):
    ...
    checkpoint_dir = os.path.join(best_config.result_dir, exp_name, 'best_trial')
    ...
    prepare_retrain_config(best_config, best_log_dir, no_retrain_optimal)
...
def main():
    ...
    parser.add_argument('--no_retrain_optimal', action='store_true',
                        help='Do not retrain with the optimal number of epochs.')
    ...
    retrain_best_model(exp_name, best_config, best_log_dir, args.no_retrain_optimal)
